...
#Вычисления примера 6
def Lab_6(polinom_1, polinom_2, prib):
    e = 0.0000001
    #cюда запишем окончательный корень
    toch = [0, 0]
    #производные функций
    Fdx = Lab_5X(polinom_1)
    Fdy = Lab_5Y(polinom_1)
    Gdx = Lab_5X(polinom_2)
    Gdy = Lab_5Y(polinom_2)
    #Якобиан
    J = Lab_4(Fdx, prib[0], prib[1]) * Lab_4(Gdy, prib[0], prib[1]) - Lab_4(Fdy, prib[0], prib[1]) * Lab_4(Gdx, prib[0], prib[1])
    #первое приближение к корню
    toch[0] = prib[0] - (1 / J) * (Lab_4(Gdy, prib[0], prib[1]) * Lab_4(polinom_1, prib[0], prib[1]) - Lab_4(Fdy, prib[0], prib[1]) * Lab_4(polinom_2, prib[0], prib[1]))
    toch[1] = prib[1] - (1 / J) * (Lab_4(Fdx, prib[0], prib[1]) * Lab_4(polinom_2, prib[0], prib[1]) - Lab_4(polinom_1, prib[0], prib[1]) * Lab_4(Gdx, prib[0], prib[1]))
...
#Запуск задания 6
def Laba6():
    polinom1 = PolinomInputXY()
    print("Введенный первый полином: ", end='')
    PolinomOutputXY(polinom1)
    polinom2 = PolinomInputXY()
    print("Введенный второй полином: ", end='')
    PolinomOutputXY(polinom2)
    prib = [0, 0]
    print("Введите приближенные х и у: ")
    for i in range (len(prib)):
        prib[i] = float(input())
    toch = Lab_6(polinom1, polinom2, prib)
